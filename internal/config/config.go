...
type ClaudeClientConfig struct {
	Model string            `mapstructure:"model"` // claude-opus-5-5 (default), or aliases: opus, sonnet, haiku
...
type CodexClientConfig struct {
	Model string `mapstructure:"model"` // gpt-6-sol (default), gpt-6-luna
}
...
type GeminiClientConfig struct {
	Model string `mapstructure:"model"` // gemini-3.8-flash (default), gemini-3.1-pro-preview
}

// OpenCodeClientConfig holds OpenCode-specific settings.
type OpenCodeClientConfig struct {
	Model string `mapstructure:"model"` // anthropic/claude-opus-5-5 (default)
}
...
orchestration:
  ...
  claude:
    model: claude-opus-5-5  # claude-opus-5-5 (default); aliases like opus, sonnet, haiku also work

  # Codex-specific settings (only used when client: codex)
  codex:
    model: gpt-6-sol  # gpt-6-sol (default)
  ...
  opencode:
    model: anthropic/claude-opus-5-5  # anthropic/claude-opus-5-5 (default)
